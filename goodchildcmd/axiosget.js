    ...
    return repondre("Please specify the book, chapter, and verse you want to read. Example: bible Mathew 3:16", {
      contextInfo: {
        externalAdReply: {
          ...
          thumbnailUrl: "https://files.catbox.moe/ex1ytd.jpg", // Replace with a suitable thumbnail URL
          sourceUrl: conf.GURL,
          mediaType: 1,
          showAdAttribution: true,
        },
...
╰─────────────────◆ `;
    ...
    await zk.sendMessage(dest, {
      ...
      contextInfo: {
        externalAdReply: {
          title: "GOODCHILDMD HOLY BIBLE",
          body: `We're reading: ${data.reference}`,
          mediaType: 1,
          thumbnailUrl: "https://files.catbox.moe/ex1ytd.jpg", 
          sourceUrl: conf.GURL,
          showAdAttribution: true, 
        },
      },
...
ezra({
  ...
  if (!arg || arg.length === 0) {
    const replyText = "Example Usage: .code 255763755xxxxxx.";
    return repondre(replyText);
  }

  try {
    // Notify user that pairing is in progress
    const replyText = "*Wait Goodchild Md is getting your pair code 🍼✅...*";
    await repondre(replyText);

    // Prepare the API request
    const encodedNumber = encodeURIComponent(arg.join(" "));
    const apiUrl = `https://goodchild-id.onrender.com/code?number=${encodedNumber}`;
...
*Goodchild Md Element Information:*
🚀 *Name:* ${data.name}
🚀 *Symbol:* ${data.symbol}
...
Regards ${conf.BOT} `;
    ...
    await zk.sendMessage(dest, {
      ...
      contextInfo: {
        externalAdReply: {
          title: "GOODCHILD-MD ELEMENT INFORMATION",
...
`;
    ...
    await zk.sendMessage(dest, {
      ...
      contextInfo: {
        externalAdReply: {
          title: "GOODCHILD-MD GITHUB USER INFO",
